...
class ObjectDetectionModel:
    ...
    def _run_inference(self, input_tensor_expected: np.ndarray) -> Tuple[Any, Any, Any, int]:

        self._interpreter.set_tensor(self.input_details_dic['index'], input_tensor_expected)
        self._interpreter.invoke()
        bounding_boxes_coordinates_list = self._interpreter.get_tensor(self.output_details_list_dic[0]['index'])[0]
        predicted_classe_indexes_list = self._interpreter.get_tensor(self.output_details_list_dic[1]['index'])[0]
        proba_of_class_predicted_list = self._interpreter.get_tensor(self.output_details_list_dic[2]['index'])[0]
        number_of_predictions = int(self._interpreter.get_tensor(self.output_details_list_dic[3]['index'])[0])

        return bounding_boxes_coordinates_list, predicted_classe_indexes_list, proba_of_class_predicted_list, \
